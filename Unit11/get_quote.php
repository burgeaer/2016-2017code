...
if (isset($_GET['symbol']) && !empty($_GET['symbol'])) { // Handle the form.
    ...
    // Identify the URL:
    $url = sprintf('https://api.iextrading.com/1.0/stock/%s/quote', $_GET['symbol']);
    ...
    if (strcasecmp($read[0], $_GET['symbol']) !== 0) {
        ...
        // Print the results:
        echo '<div>The latest value for <span class="quote">' . $read[1] . '</span> (<span class="quote">' . $_GET['symbol'] . '</span>) is $<span class="quote">' . $read[7] . '</span>.</div>';
